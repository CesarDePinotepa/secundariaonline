...
if (isset($_POST['resTxa']) && !empty($_POST['resTxa'])) {
    ...
    if ($ejecutar) {
        ...
        header("Location: ../../vistas/miforo/comen-e.php?id=$id&bien=$bien");
    }else{
        $error = "No se pudo registrar el curso. Error:". mysqli_error($conexion);
        header("Location: ../../vistas/miforo/comen-e.php?id=$id&err=$error");
    }
}else{
    header("Location: ../../vistas/miforo/comen-e.php?id=$id");
}
